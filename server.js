...
'[BLOCKS_START]\n' +
'[{"type":"incident_card","data":{"severity":"P1","category":"logistics","title":"<event title, max 60 chars>","subtitle":"<one-line event summary, max 60 chars>","location":{"display":"<REGION · LOCATION, uppercase, max 25 chars, e.g. SOUTH CHINA · YANTIAN>","primary":"<primary marker name, uppercase, e.g. YANTIAN>","sub":"<sub label, uppercase, e.g. SHENZHEN>"},"direct_exposure_usd":<estimated USD number, use 0 if unknown>,"affected_pos_count":<estimated count, use 0 if unknown>,"suppliers_hit":[{"name":"<supplier name relevant to user profile or disruption location>","exposure_usd":<number, use 0 if unknown>}],"delay_risk":"<e.g. 9-14 days to North America DCs>","recommended_action":{"text":"<one concrete sentence recommending an action>","impact_estimate":"<brief estimated benefit, e.g. Saves ~$180K in delay costs>","cta_label":"Run mitigation"},"recommendedActions":[{"text":"<action description, plain text only, no HTML or asterisks, max 120 chars>","impact":"<primary impact tag, e.g. Preserves $X · saves Y>","impact2":"<optional second impact line for item 0 only>","cta":"<button label, uppercase, max 20 chars, e.g. INITIATE REROUTE>"},{"text":"<second action, plain text>","impact":"<impact>","cta":"<button label>"},{"text":"<third action, plain text>","impact":"<impact>","cta":"<button label>"}],"trackingFooter":{"update_every":"5 min","next_refresh":"05:00","escalation":"Ops Lead"}}},{"type":"follow_up_chips","options":["<3-6 word chip>","<3-6 word chip>","<3-6 word chip>"]}]\n' +
'[BLOCKS_END]\n\n' +
'When you include an incident_card, the location field is required. Use short uppercase labels: display is REGION · LOCATION (max 25 chars), primary is the port or city name, sub is the broader city or region.\n\n' +
'When you include an incident_card, both recommendedActions (exactly 3 items) and trackingFooter are required. Action text must be plain text — no HTML, no asterisks, no markdown. Prioritize concrete supply chain actions: rerouting POs, holding shipments, notifying suppliers. impact2 is optional and for item 0 only. For trackingFooter, copy the exact values shown in the template.\n\n' +
